...
async function scrollToLoadPosts(page, targetCount = 100) {
  ...
  for (let attempt = 0; attempt < 30; attempt++) {
    ...
    if (count === lastCount && attempt > 0) break; // no new posts loaded
    lastCount = count;
      ...
      try {
        ...
        for (let i = 0; i < likeCount; i++) {
          try {
            ...
            if (alreadyLiked) {
              console.log(`  Post ${i + 1}: already liked @${owner || "unknown"}, advancing.`);
            } else {
              ...
              console.log(`  Post ${i + 1}: liked @${owner || "unknown"} (${locationLiked} for location ${locationId})`);
            }
